...
class HttpBusinessRepository implements BusinessRepository {
    ...
    return [
      for (final row in rows)
        if (row is Map<String, dynamic>)
          ...
          throw const ApiException(ApiFailure.malformed, 'expected business objects'),
    ];
  }
  ...
  Future<BusinessModel> registerBusiness({
    ...
    final created = await _api.post('/businesses', body: {
      'name': name,
  ...
  static BusinessCategory _category(String raw) {
    ...
    throw ApiException(ApiFailure.malformed, 'unknown business category "$raw"');
  }
