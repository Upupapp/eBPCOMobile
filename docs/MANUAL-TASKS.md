...
| M-47 | **Reconcile `POST /applications` between the contract and this app.** Measured 29 Aug 2026 by diffing the request body against the contract's `ApplicationSubmission` schema, which is `additionalProperties: false` — so one undeclared key rejects the whole submission. The app would be refused on **four** counts: (a) the required `serviceDomain` is never sent and the app has no notion of it; (b) it sends `documents` (local labels and filenames) where the contract declares `documentIds` (uuids of files already uploaded via `/documents`, a flow that is not built); (c) `businessId` is `''` for every construction permit, and the contract types it as a uuid or null; (d) **not one of the 19 permit types the app files is in the contract's enum**; only `Certificate of Occupancy` matches, and only by spelling. **Corrected on measurement:** this is not a stale contract — `ebpco-api`'s `permit_types` table carries the same 17 short names, so it is admin+mobile (19 canonical) against contract+server (17 short). 15 pairs differ only in spelling; **3 mobile types have no server row at all** (Zoning / Locational Clearance, FSEC, FSIC — TABs 03/04/05 built wizards for permits the server does not know exist); and the server's `Business Permit` row is a service domain in mobile's model. `permit_types.permit_type` is a primary key with `document_requirements` referencing it, so this is not a rename. **The fix is not to change this app**: mobile matches the admin exactly, proven by the standing vocabulary gate, and adopting the contract's spelling would re-open every lookup keyed on the office's own names. Asserted in `test/contract/application_submission_test.dart`, which fails the day it is reconciled. Full hand-off with the recommendation: `docs/HANDOFF-M-47-permit-vocabulary.md` **Widened 29 Aug after auditing the remaining write paths: it is three of the five paths a client calls, not one.** `POST /applications/:id/payments` omits the required `paidOn` — the app never asks the applicant when they paid, so closing it needs a question added to the payment flow, not just a key — and sends `proof: {label, fileName}` where the contract declares `documentId`. `POST /applications/:id/instructions/:letterId/resubmit` sends **no body at all** against a required `items`; this is the route the 28 Aug remeasure called "exists" when marking M-43 closed — it does exist, and the app calls it wrongly, which nothing compared. `POST /businesses`, `/auth/register` and `/auth/token` conform exactly and are asserted so a regression fails. All six bodies are gated in `test/contract/write_bodies_test.dart` against the vendored `test/contract/write-bodies.json` | Contract lane, with the backend and admin lanes; a contract change, so a version bump |
