...
| M-48 | **Persist wizard drafts — 2 of 19 done, 17 to go.** Measured 29 Aug 2026: nineteen wizards offered **Save as Draft** and **nothing was written to disk**; every draft lived in a `ChangeNotifier` and died with the process, so an applicant who filled seven of a Building Permit's nine steps and closed the app lost all of it, attachments included. **Built 30 Aug 2026** for **Building Permit and Fencing**: `lib/core/drafts/` snapshots a draft to the keychain on every Save as Draft and reads it back on launch. Attachments are deliberately NOT persisted — a picked file's path is not reliably readable after a restart — and are instead named back to the applicant on the Drafts row (*"Re-attach 3 documents: …"*). Guarded by `test/architecture/draft_snapshot_completeness_test.dart`, which fails if a field is captured and not read back, read back and not captured, or invented; it was falsified five ways before being believed. **What is left is repetition, not design**: one codec per remaining wizard plus one line in the gate's list. The applicant-facing copy stays narrowed until all nineteen persist, and a test gates the sentence on the converted count so it cannot be relaxed early. See `docs/M-48-draft-persistence-part-1.md` and `docs/SCOPING-M-48-draft-persistence.md` | Front-end work, no longer blocked on a decision |
