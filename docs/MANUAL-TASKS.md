...
| M-48 | **Persist wizard drafts, or keep the narrowed promise.** Measured 29 Aug 2026: nineteen wizards offer **Save as Draft**, nineteen providers implement `DraftSource`, the Applications tab has a **Drafts** segment listing them and a "Last saved" timestamp — and **nothing is written to disk**. Every draft lives in a `ChangeNotifier` and dies with the process; only `AuthProvider` persists anything. An applicant who fills seven of a Building Permit's nine steps, taps Save as Draft and closes the app loses all of it, including up to 22 attached documents. The Before-you-start card promised the opposite and has been narrowed to say what is actually true. Implementing it is a feature, not a fix: nineteen bespoke draft classes, each with five to nine sections and `DocumentModel` references, need serialising — and drafts carry an applicant's name, address and identity-document filenames, so the store must be the keychain rather than SharedPreferences (see M-01, M-32). Proven and guarded by `test/features/applications/draft_persistence_test.dart`, which fails the day persistence arrives so the copy can be restored Scoping measured in `docs/SCOPING-M-48-draft-persistence.md`: 236 classes, 2,624 typed fields, 586 document slots; four options with a recommendation | Owner call on scope; then front-end work |
