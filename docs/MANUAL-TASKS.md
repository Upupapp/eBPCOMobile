...
| M-29 | **Confirm final bundle identifiers.** `com.ebpco.ebpcoUserApp` (iOS) and `com.ebpco.ebpco_user_app` (Android) are development placeholders, and **they do not match each other** — two spellings naming one product. Permanent after publication: neither store lets an app change its identifier, and a new one is a new listing. It also blocks every remaining iOS step, because registering a provisioning profile binds the identifier to the team, which is why no archive has been attempted. Measured and prepared in `docs/DECISION-M-29-bundle-identifier.md`: 12 files across 6 targets, the iOS half is 6 entries in one Xcode project and nothing in `lib/` reads it, the Android half moves a package directory and belongs to the Windows lane. **The domain is the LGU's to state — no `.gov.ph` domain for Castilla is recorded anywhere in this repository, and a guess would put fabricated authority into a permanent identifier** | Owner call; say the identifier and the iOS half is three lines |
