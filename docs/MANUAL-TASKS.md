...
| M-43 | **Provide the document-resubmission route.** The app now offers an applicant a Replace action on any document the office rejected or sent back, and posts the replacement to `POST /applications/:id/documents/:documentId/resubmit`. That route does not exist. Until it does, resubmission works against the mock only and fails loudly on a live build — deliberately, rather than reporting a success that did not happen | Backend lane; needs the document id the portal uses |
