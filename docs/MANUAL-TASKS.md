...
| M-41 | **Decide whether to raise the 1.3x text-scale clamp in `app.dart`.** The app clamps system font scaling to a maximum of 1.3x. The comment says why: layouts were built against a 1.0x browser preview and overflowed on Android at higher scales, so the clamp works around the bugs rather than fixing them. Those bugs are now largely fixed — 200% renders clean across all 16 wizards, all 16 shared widgets and 30 screens — so the clamp is mostly capping accessibility for no remaining reason. Raising it is a user-visible change both lanes should see before it ships | Owner call; needs the Android lane's visual QA, and the ~16 still-uncovered screens checked first |
| M-42 | **Decide what "View Application" should do on the submitted screens.** Fourteen confirmation screens show "View Application" and "Return to Applications" side by side, and both go to `/app/applications`. The first was almost certainly meant to open the detail page for the application just filed, but these screens are passed a reference number and not an application id, so the route cannot be built from what they have. Either pass the id through at submission, or drop the redundant button | Needs a product call plus a wizard change to pass the id |
