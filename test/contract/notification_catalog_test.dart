...
void main() {
  ...
  String wireNameOf(String constant) =>
  ...
  Set<String> declaredRoutes() {
    ...
    return RegExp(r"path: '(/applications/:applicationId[^']*)'")
        .allMatches(source)
  ...
  group('contract: notification catalog', () {
    ...
    test('the server carries exactly this app’s types', () {
      final fromContract = catalog.map((entry) => entry['type'] as String).toSet();
      final fromApp = NotificationType.values.map((type) => wireNameOf(type.name)).toSet();

      expect(fromApp, hasLength(25));
      expect(
        fromContract,
        fromApp,
        reason: 'a type in only one of the two is a notification one side can send '
    ...
    test('every type carries the same category the app assigns it', () {
      ...
      final byWire = {for (final entry in catalog) entry['type'] as String: entry};

      for (final type in NotificationType.values) {
        final entry = byWire[wireNameOf(type.name)];
        expect(entry, isNotNull, reason: '${type.name} is missing from the contract');
        expect(
          entry!['category'],
          type.category.name,
          reason: '${type.name} is ${type.category.name} here and ${entry['category']} on the server',
        );
      }
    });
    ...
    test('requiresAction matches this app’s action priority', () {
      ...
      for (final type in NotificationType.values) {
        expect(
          ...
          reason: '${type.name} disagrees with the server on whether it needs an act',
        );
      }
    });
    ...
    test('an action-required notification points at the screen that resolves it', () {
      final byWire = {for (final entry in catalog) entry['type'] as String: entry};

      expect(byWire['order-of-payment-issued']!['deepLink'], contains('/pay'));
      expect(byWire['revision-required']!['deepLink'], contains('/instructions'));
      expect(byWire['letter-of-instruction-issued']!['deepLink'], contains('/instructions'));
      expect(byWire['ready-for-release']!['deepLink'], contains('/permit'));
    });

    test('the two this app derives locally are marked as never sent by the server', () {
      // A draft is local until it is filed, and a credential expiry is computed
      // from records held on the device.
      final clientOnly = catalog
