...
void main() {
  ...
  test('document counts, required counts, validity and office all match', () {
    for (final entry in requirementsCatalog.entries) {
      ...
      expect(
        actual.requiredDocumentCount,
        expected.required,
        reason: '${entry.key.wire} required count',
      );
      expect(
        actual.validityMonths,
        expected.validity,
        reason: '${entry.key.wire} validity',
      );
      expect(
        actual.responsibleDepartmentId,
        expected.dept,
  ...
  test('the three offices are represented', () {
    ...
    final offices = requirementsCatalog.values
        .map((e) => e.responsibleDepartmentId)
        .toSet();
